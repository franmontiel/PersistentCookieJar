...
import com.franmontiel.persistentcookiejar.persistence.SharedPrefsCookieResistor;
...
public class PersistentCookieJarTest {
    ...
    private final HttpUrl url = HttpUrl.parse("https://domain.com/");

    @Before
    public void createCookieJar() {
        persistentCookieJar = new PersistentCookieJar(
                new SetCookieCache(),
                new SharedPrefsCookieResistor(RuntimeEnvironment.application.getApplicationContext())
        );
    }
    ...
    public void differentUrlRequest() {
        ...
        List<Cookie> storedCookies = persistentCookieJar.loadForRequest(Objects.requireNonNull(HttpUrl.parse("https://otherdomain.com")));
    ...
    public void clearSessionCookies() {
        ...
        assertEquals(1, persistentCookieJar.loadForRequest(url).size());
